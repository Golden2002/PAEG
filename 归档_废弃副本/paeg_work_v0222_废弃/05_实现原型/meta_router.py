...
──── v0.36 命名规范（针对历史审计 P0-E）────
本模块的对外函数名/返回值命名遵循"选项名 = 规则变量名"原则：
  - is_teaching_intent() → 返回 bool（True/False，不再用 teaching/non_teaching 二元变量名）
  - is_meta_question() / is_greeting() / is_problem_request() / is_method_advice()
    / is_knowledge_query() / is_affection_expression() → 全部 bool 返回
  - 历史上若出现过 teach / teaching / non_teaching 三套命名（v0.24 旧 route()），
    已废弃；当前统一为 is_*_intent / is_* 形式。server.py 调用方注意：
    `from meta_router import is_teaching_intent` 即可，旧 route() 不再依赖。
"""
