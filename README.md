...
> 用户顶级指令：四大工具（语言规范校对/外语词汇表/法律检索/通用简历）按**顶尖标准总控**分五波次推进（初始化锚定→方案设计→核心开发→双审计联调→发布迭代），每波次前出具审计报告。**目录铁律**：保持现有 14.x 本地目录结构（不新建项目文件夹），`D:\项目\` 为总控工作区（波次计划/审计报告/基线锚定副本/能力清单/对标表/需求规格）。

**波次 1 完成状态**（审计通过，放行波次 2）：

| 交付物 | 位置 | 状态 |
|---|---|---|
| 基线锚定 5 份（文件级 100%） | `D:\项目\01~04\baseline\` | ✅ 词汇表@32539f9(117) / 法律@5a1e747(91) / 简历主@2b13c92(274) / 简历辅@f5bf6f6(271) / 语言规范模块(6) |
| 基线能力清单 | `D:\项目\*\docs\03_基线能力清单*.md` | ✅ 词汇表 50 项 / 法律约 100 项 / 简历主双形态 616 用例 / 简历辅 82 项 / 语言规范 L-01~L-12 |
| 顶尖标准对标表 | `D:\项目\*\docs\02_顶尖标准对标表.md` | ✅ 4 份（蜜度文修 / Vocabu-Lexus-单词鸭 / Harvey-北大法宝AI / Rezi-WonderCV） |
| 需求规格 v1.0 | `D:\项目\*\docs\01_需求规格说明书_v1.0.md` | ✅ 4 份（含校验清单） |
| 波次 1 审计报告 | `D:\项目\*\docs\04_审计报告_波次1.md` | ✅ 通过（路径合规/基线完整/熔断未触发） |

**波次 2 待启动**：技术架构设计（每工具）/ 简历双基线融合方案 / 网页端产品方案 / Oracle 论证 / 需求 v2.0。
