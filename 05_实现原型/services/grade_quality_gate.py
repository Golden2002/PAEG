...
# §3.79 Round 11/12 ⭐ 内容深度五要素（教学输出强化，借鉴张宇扬课件质量特征：
# 精确概念定义 → 机制解释 → 具体例子 → **真实数据锚点** → 小结/衔接；
# Round 12 新增"数据"要素：张宇扬课件"真实数据例题"特征——数字/数据/实验锚定）
_DEPTH_FEATURES: Dict[str, str] = {
    "定义": r"定义|概念|是指|称为|指的是",
    "机制": r"为什么|原理|机制|因为|本质|推导",
    "例子": r"例子|例如|比如|实例|案例|例题|生活",
    "数据": r"\d|数据|数值|实验|统计|测量|观测",
    "小结": r"小结|总结|回顾|总之|因此|所以",
}
_DEPTH_MIN: int = 3  # 五要素命中 ≥3 视为达标（宽松防误伤）
