...
# v0.71 ⭐ 病句确定性修正（用户反馈："我在这里听着你"是病句——"听"后缺补语）。
# 设计：不依赖 AI 味检测/LLM 改写（概率性），用正则做确定性兜底（规则层）；
# 只修**悬空的"听着你"**（后接句末/停顿标点）——"听着你说/听着你讲/听着你的话/
# 听着你呼吸"等已带补语/后接成分的合法搭配全部保持原样。
# 替换目标遵循用户规范："我就在这里听你说说/讲一讲"。
_GAFFE_END = r'(?=[。！？!?，,；;：:…]|$)'
_GAFFE_FIX_RULES = (
    # ① 最典型："我在这里听着你。"（用户反馈原句）
...
def fix_known_gaffes(text: str) -> str:
    """v0.71 ⭐ 病句确定性修正（规则兜底，语言规范 L0 前置层）。
