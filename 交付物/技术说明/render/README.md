...
**24. Mermaid 幽灵块根因（v1.1.9 · 2026-08-16）——最深刻的教训**：
- **症状**："图26 渲染泄漏"——PDF 中 material_pipeline 源码（`%%{init} flowchart LR IN["主题输入"]...`）原样输出在 P31；浏览器加载 28/28 SVG 全部正常。
- **排查浪费**：怀疑渲染器/图内容/位置/序号/init/菱形/br/mmdc 预渲染/PNG 嵌入——全试遍无效（换位置/序号/内容都不影响"泄漏跟随图26"）。**两天试错，根因其实是一个字符的问题**。
- **根因**：文档中混入一个**幽灵块**——swap 图实验残留的图26 副本，开头**缺 ```` ```mermaid ```` 前缀**（只有裸 `mermaid` 行）。build_html 的正则 `re.sub('```mermaid\n(.*?)```')` 匹配不到它 → 它作为**普通文本**进 HTML → Chromium print 原样输出 → 看起来像"图26 源码泄漏"。
- **为什么之前所有实验无效**：问题根本不在图26（它一直渲染正常），而在文档里的孤儿文本。改图26 的任何属性当然无效——"泄漏跟随图26 内容"只是因为幽灵块就在图26 副本附近。
- **排查方法（关键）**：比较"成功版本 vs 失败版本"的**原始 markdown**（`git show <commit>:文件.md`），而非在失败版本上反复改。一对比就发现失败版多了一个无围栏前缀的孤儿块。
- **预防**：①渲染前扫描"裸 mermaid 行"（`^\s*mermaid\s*$`，无 ``` 前缀）→ 有即中止；②所有 mermaid 块必须 ```` ```mermaid ```` 开头；③`git show` 对比法优先于试错法。
- **教训**：①**先怀疑文档结构，再怀疑渲染流程**——浏览器正常 + PDF 异常 = 问题在源文档而非工具；②复杂问题先找"成功 vs 失败"的差异，不要盲目改；③一个简单的错误（缺 3 个字符）可以改两天——**排查方法比排查时长更重要**。
...
| 2026-08-16 | v1.1.9 | Mermaid 幽灵块根因修复（删无 ```mermaid 前缀的图26 残留副本，纯文本进 HTML 致 print 泄漏）+ 图册工具链（extract 幽灵块检测/pre_render/verify 质量门/build 流水线）+ 原方法回归（44 页零泄漏）+ 渲染经验 #24 |
