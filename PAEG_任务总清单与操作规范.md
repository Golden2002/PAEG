...
| **NEW-29** | **§3.79 张宇扬课件入库 + 问句去 AI 味 + 技术说明引用（Round 12/13）✅ 已完成**：①张宇扬课件 → `Library/common/张宇扬课件/`（7 门生物课程 466MB，公共 scope 可检索；.gitignore 排除大文件；README 记录来源与质量特征）②绕出问句去 AI 味（完整句式"我们接下来是继续学习这个新话题，还是回去接着刚才的内容学习？你随时告诉我你的想法就可以。"）③内容深度守门新增"数据"要素（张宇扬真实数据例题特征）④技术说明 C.22-C.25 补齐 + 引用 Chinese-Teaching-AI-Agent/知识增强教案/EduPlanner/LectūraAgents/Mini Slide Generator | 用户指示"张宇扬课件移到 Library 公共目录作为知识库；问句补主语状语修饰语；技术说明引用参考项目"；问句/深度/意图测试 21/21 + 全回归 197+ 全绿 |
