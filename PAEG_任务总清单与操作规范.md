...
1. **检测**：三 Oracle 测试工程教学流（主题=外汇管制与通胀，与标杆同题），对比 PAEG 输出 vs 标杆
2. **差距分析**：受众定位/概念精度/结构层次/深度/类比/边界条件 六维对比
3. **对策**：Oracle 咨询提升方案（提示词/presenter 架构/检查器）
4. **实施 + 验证**：改后重测对比
5. **物料质量确认**：6 类物料三 Oracle 评分确认
